from langchain_core.messages import SystemMessage, HumanMessage

from src.core.state import AgentState
from src.core.nodes.utils import llm, scored_docs_to_context, has_usable_docs, format_node_output


NO_EVIDENCE_ACCIDENT = """
검색된 유사 사고 사례가 없습니다.
"""


ACCIDENT_SYSTEM_PROMPT = """
당신은 검색된 사고 사례 정리자입니다.
- 검색 문서 밖의 내용은 쓰지 마십시오.
- 조건과 수치는 바꾸거나 일반화하지 마십시오.
- 사고 사실만 정리하십시오. 판단, 대책, 권고는 금지합니다.
- 표, 문서 ID, 페이지, 점수, 참고 문구는 금지합니다.
- 문서가 비면 "검색된 유사 사고 사례가 없습니다"만 출력하십시오.
"""


ACCIDENT_USER_TEMPLATE = """
[설비 정보]
{facility_info}

[검색된 사고 사례]
{context}
"""


def accident_analysis_node(state: AgentState):
    """
    [노드: 사고 사례 분석]
    Accidents 테이블(키워드 매칭)에서 가져온 유사 사고 사례를 분석합니다.
    """
    intents = state.get("target_intents") or []
    if "risk" not in intents:
        return {}

    print(">>>>> [NODE] Accident Node <<<<<")
    docs = state.get("accident_docs") or []
    existing = list(state.get("partial_answers") or [])

    if not has_usable_docs(docs):
        msg = NO_EVIDENCE_ACCIDENT.strip()
        return {
            "partial_answers": existing + [msg],
            "node_outputs": {"accident": msg},
        }

    messages = [
        SystemMessage(content=ACCIDENT_SYSTEM_PROMPT),
        HumanMessage(
            content=ACCIDENT_USER_TEMPLATE.format(
                facility_info=state.get("facility_info", ""),
                context=scored_docs_to_context(docs),
            )
        ),
    ]
    res = llm.invoke(messages)

    content = format_node_output((res.content or "").strip(), docs)
    return {
        "partial_answers": existing + [content],
        "node_outputs": {"accident": content},
    }
