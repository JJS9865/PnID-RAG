...
COMMON_CONFIG = {
    ...
    "pretraining_data_path": "./data/dataset/processed/pretraining_data_2026.03.14.jsonl",
    "instruction_data_path": "./data/dataset/processed/instruction_data_2026.03.05.jsonl",
...
def get_config(prefix):
    stage_config = P_CONFIG if prefix == "p" else I_CONFIG
    merged = {**COMMON_CONFIG, **stage_config}
    merged["data_path"] = (
        COMMON_CONFIG["pretraining_data_path"]
        if prefix == "p"
        else COMMON_CONFIG["instruction_data_path"]
    )
    return merged
...
def train():
    ...
    prefix = get_stage_prefix(args.stage)
    CONFIG.update(get_config(prefix))  # P 또는 I 전용 설정 병합
    ...
    sample_pct = CONFIG["data_sample_ratio"]
    ...
    sft_kwargs = dict(
        output_dir=checkpoint_dir, num_train_epochs=CONFIG["epochs"],
    ...
    if CONFIG.get("neftune_noise_alpha") is not None:
